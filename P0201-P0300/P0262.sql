...
SELECT Request_at AS Day,
       ROUND(COUNT(CASE WHEN Status IN ('cancelled_by_driver', 'cancelled_by_client') THEN Status END) / COUNT(*), 2)   AS 'Cancellation Rate'
FROM (SELECT Status, Request_at
      FROM Trips
      WHERE EXISTS(SELECT Users_Id FROM Users WHERE Trips.Client_Id = Users.Users_Id AND Users.Banned = 'No')
        AND EXISTS(SELECT Users_Id FROM Users WHERE Trips.Driver_Id = Users.Users_Id AND Users.Banned = 'No')) AS t
