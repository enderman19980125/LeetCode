...
class Solution:
    def deleteDuplicates(self, head: ListNode) -> ListNode:
        ...
        pre_value = h.val
        while p:
            if p.val != pre_value and (p.next is None or p.val != p.next.val):
                pre_node.next = p
                pre_node = p
            pre_value = p.val
            p = p.next
...
if __name__ == '__main__':
    ...
    first_node = to_link([1, 2, 3, 3, 4, 4, 5])
    answer = solution.deleteDuplicates(first_node)
    print(to_string(answer), "1->2->5")

    first_node = to_link([1, 1, 1, 2, 3])
    answer = solution.deleteDuplicates(first_node)
    print(to_string(answer), "2->3")
