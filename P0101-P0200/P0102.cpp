    ...
    vector<vector<int>> levelOrder(TreeNode *root) {
        ...
        while (!level_nodes[k].empty()) {
            ...
            for (auto &node:level_nodes[k]) {
                ...
                if (node->left) {
                    level_nodes[1 - k].push_back(node->left);
                }
                if (node->right) {
                    level_nodes[1 - k].push_back(node->right);
                }
            }
            level_nodes[k].clear();
...
int main() {
    ...
    answer = solution.levelOrder(root_node);
    cout << toString(answer) << " " << "[[3], [9, 20], [15, 7]]" << endl;
    ...
    root_node = toTree(nodes);
    answer = solution.levelOrder(root_node);
    cout << toString(answer) << " " << "[[1], [2], [3], [4, 5]]" << endl;
